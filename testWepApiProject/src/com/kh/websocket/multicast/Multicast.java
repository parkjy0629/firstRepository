...
// 			이후 해당 값을 @PathParam 이라는 어노테이션을 통해 해당 값을 사용할 수 있다.
...
public class Multicast {
	...
	// 각 방 별로 사용자들의 정보를 담기 위한 Map 객체를 선언하여 각 방을 key로 사용자
	// 리스트를 Set<Session> 컬렉션 객체로 담아서 서버 사용자들의 접속 정보를 통합
	// 관리하기 위한 객체이다.
   ...
   public static Set<Session> getUserList(String room){
   	...
   	// 연결하려는 사용자의 정보와 접속한 방의 이름을 받아서 해당 방의 사용자 리스트에
   	// 갱신을 해주어야 한다.
    @OnOpen
    public void onOpen(EndpointConfig config, Session session, @PathParam("chatRoom") String room) {
    	// EndpointConfig : 기본 설정을 적용하겠다.
    	// Session : 현재 접속자의 소켓 정보를 담은 연결 객체 (HttpSession 과는 다르다)
    	// @PathParam : 동적인 url 경로를 통해 접근할 경우 해당 경로 부분을  변수로 활용할 수 있게
    	//						매개변수로 변경하여 받을 수 있는 어노테이션
    	
    	// session.getUserProperties() : 웹 소켓 세션에 사용자가 별도로 추가하거나 수정할 수 있는 설정 부분을 
    	//											가져올 때 사용하는 메소드
    	// getUserProperties() 해당 설정을 통해 웹 소켓 session 내부에 사용자의 ID와 해당 방의
    	// 이름을 저장하여 이후에 꺼내어 사용할 수 있게 등록한다.
       session.getUserProperties().put("chat_id", config.getUserProperties().get("chat_id"));	// HttpSession 과 웹소켓 세션이 다르기 때문에 HttpSession의 정보를 가져오기 위해 config로 설정한 GetHttpSession이 필요
    ...
    public void onMessage(String message, Session session) throws IOException {
    	...
    	// 현재는 사용하지 않지만 이후 확장 가능성 즉, 나중에 사용자 리스트를 표현하거나
    	// 어떤 사용자가 보낸 것인지 표현할 때 사용할 수 있으므로 코드를 작성해 놓은 부분
    	String chat_id = (String) session.getUserProperties().get("chat_id");
    	
    	// 현재 데이터를 보낸 사용자의 방 정보를 받아서 같은 방에 있는 사용자들에게
    	// 해당 정보를 전달하기 위한 객체
		...
		userList.stream().forEach( x -> {
			// 람다식 사용부 : 
			// 기존 절차지향 언어가 가지는 특성인 익명 함수 구현부분을 객체 지향 언어에서도
			// 흉내내기 위한 소스 코드 작성 기술
			
			// 쉽게 이해하기 위해 JavaScript의 function(){ ... } :: () -> { ... } 를 생각해보자~!
			// x : Session을 의미
		   try {
			   // 실제 데이터를 전달하는 로직 부분
		      if(!x.equals(session))
    ...
    public void onClose(Session session) {
       ...
       // 만약 방 안에 사용자가 한명도 없다면 채팅방을 폐쇠한다.
